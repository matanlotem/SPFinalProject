 ...
 * - SP_CONFIG_INVALID_BOOL - if a line in the config file contains invalid boolean
 * - SP_CONFIG_INVALID_LINE - if a line in the config file is not formatted properly
...
/*
 * Returns the number of similar images to find, as set in the configuration file,
...
/**
 * Returns the KDTree split method
 *
 * @param config - the configuration structure
 * @assert msg != NULL
 * @param msg - pointer in which the msg returned by the function is stored
 *
 * @return configured split method on success, default method otherwise.
 *
 * The resulting value stored in msg is as follow:
 * - SP_CONFIG_INVALID_ARGUMENT - if config == NULL
 * - SP_CONFIG_SUCCESS - in case of success
...
/**
 * Returns the number of images to hold in the queue - KNN
 *
 * @param config - the configuration structure
 * @assert msg != NULL
 * @param msg - pointer in which the msg returned by the function is stored
 ...
 * store the resulting string.
 * the suffix is specified in SP_FEATURES_SUFFIX
 *
 * @param imagePath - an address to store the result in, it must contain enough space.
 * @param config - the configuration structure
 * @param index - the index of the image.
 ...
 * - SP_CONFIG_SUCCESS - in case of success
 */
SP_CONFIG_MSG spConfigGetFeaturesPath(char* featsPath, const SPConfig config,
		int index);
/**
...
/*
 * Initiates the program logger (if not already initiated) based on
 * the configuration file values
 *
 * @param config - the configuration structure
 * @param loggerMsg - pointer in which the logger creation message
 * 					  returned by the function is stored
 *
 * @return
 * - SP_CONFIG_INVALID_ARGUMENT - if config == NULL or loggerMsg == NULL
 * - SP_CONFIG_SUCCESS - in case of success (logger creation can still fail)
 *
 * The resulting value stored in loggerMsg depends on the output
 * of
